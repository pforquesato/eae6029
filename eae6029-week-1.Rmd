...
The squared CEF error $\sigma^2 \equiv \mathbb{E}\left[e^2\right] = \text{Var}(e)$ is the **variance of the regression error**, and measures the magnitude of *unexplained variation* of $Y$, given $X$

$\sigma^2$ depends on $X$: if we add regressors, we can never explain less of the variance (at worst, the aditional regressors are useless, never worse than nothing) &mdash; this is reflected in the following important inequality:
...
In what follows, we will work with the matrix notation, omitting the boldface. Consider first the **projection matrix**, which projects vectors $Z$ into the *column space* of $X$:

$$P = X (X^{\prime}X)^{-1} X^{\prime}$$
Clearly, if $Z$ is already in the column space of $X$, say, $Z = X\Gamma$ for some $\Gamma$, then the projection should not move $Z$ at all! Indeed, that is true:

$$PZ = PX\Gamma = X (X^{\prime}X)^{-1} X^{\prime}X\Gamma = X\Gamma = Z$$

---
class: middle
## Projection matrix

For the same intuition, we should have that since $PZ$ is in the column space of $X$, $P(PZ)$ does not move it &mdash; this is indeed true, as the projection matrix $P$ is **idempotent**: $PP = P$

The projection matrix is important because OLS is nothing more than the projection of $Y$ on the column space of $X$, generating the fitted values $\widehat{Y} = PY$ (for this reason, $P$ is also known as **hat matrix**)

$P$ also has some cool linear algebra properties: it is symmetric: $P = P^{\prime}$; $tr(P) = rank(P) = k$, and it has $k$ eigenvalues of $1$ and $n-k$ of zero (this part not surprising, since rank is $k$)  
...
The **annihilator matrix** is defined as $M = I_{N} - P$. It is called that because it "annihilates" any $Z$ in the range of $X$:

$$MZ = MX\Gamma = (I_N - P)X\Gamma = X\Gamma - X(X^{\prime}X)^{-1}X^{\prime}X\Gamma = X\Gamma - X\Gamma = 0$$
Namely, $X$ and $M$ are orthogonal. While $P$ creates fitted values, $M$ creates *residuals*: $MY = (I_N - P)Y = Y - \widehat{Y} = \widehat{e}$

Like the projection matrix, the annihilator is idempotent and symmetric, but with rank (and trace) $n - k$: $M$ and $P$ form a basis of $\mathbb{R}^N$

---
class: middle
## Regression components

Let's finish this linear algebra incursion by partitioning $X$ in 2 subsets, such that $Y = X_1 \beta_1 + X_2 \beta_2 + e$: often we will want to estimate $\beta_2$, but running the full $k$-dimensional regression is unfeasible

We still can do so by minimizing sequentially regarding $\beta_1$ and $\beta_2$, keeping the other fixed (see textbook) &mdash; if we define $M_2 = I_N - X_2 (X_2^{\prime}X_2)^{-1}X_2^{\prime}$, then we have that the linear projection (OLS) coefficient satisfies:

$$\widehat{\beta}_2 = \arg \min_{\beta_2} (Y - X_2\beta)^{\prime}M_1(Y - X_2\beta) = (X_2^{\prime}M_1X_2)^{-1}X_2^{\prime}M_1Y$$

---
class: middle
## Frisch-Waugh-Lovell Theorem

Since $M_1$ is idempotent and symmetric,

$$\widehat{\beta}_2 = (X_2^{\prime}M_1X_2)^{-1}X_2^{\prime}M_1Y = ((X_2^{\prime}M_1^{\prime})(M_1X_2))^{-1}(X_2^{\prime}M_1^{\prime})(M_1Y)$$

$$\therefore \widehat{\beta}_2 = (\widetilde{X}_2^{\prime}\widetilde{X}_2)^{-1}\widetilde{X}_2^{\prime}\widehat{e}_1,$$

where $\widetilde{X}_2 = M_1 X_2$ and $\widehat{e}_1 = M_1 Y$ (remember, $M$ is the annihilator): and thus $\widehat{\beta}_2$ is the coefficient of a regression of $\widetilde{X}_2$ on $\widehat{e}_1$

---
class: middle
## Frisch-Waugh-Lovell Theorem

Now, since $M_1$ is the annihilator matrix of $X_1$, that generates *residuals* of a regression on $X_1$, we have that $\widetilde{X}_2$ is the residuals of $X_2$ on $X_1$, and $\widehat{e}_1$ the residuals of $Y$ on $X_1$
...
2. Regress $X_2$ on $X_1$ and obtain residuals (of size $N\times K_1$) $\widetilde{X_2}$
...
$$\mathbb{E} \left[ \widehat{\beta} | X \right] = \mathbb{E} \left[ \left( \mathbf{X}^{\prime}\mathbf{X}\right)^{-1} \mathbf{X}^{\prime}\mathbf{Y}\ |\ \mathbf{X} \right]$$


$$=  \left( \mathbf{X}^{\prime}\mathbf{X}\right)^{-1} \mathbf{X}^{\prime} \mathbb{E} \left[ \mathbf{Y} | \mathbf{X} \right] = \left( \mathbf{X}^{\prime}\mathbf{X}\right)^{-1} \mathbf{X}^{\prime}\mathbf{X}\beta = \beta$$
...
Finally, by the **LIE**, $\mathbb{E} \left[ \widehat{\beta} \right] = \mathbb{E} \left[ \mathbb{E} \left[ \widehat{\beta} | X \right]\right] = 0$ $\blacksquare$
...
If we assume errors are correlated among clusters, now for identification we need that $\mathbf{E} \left[ e_{ig} | \mathbf{X}_g \right] = 0$, for all $i$ in group $g$ &mdash; given this, however, OLS is still unbiased, but now the variance-covariance matrix must be estimated to be **cluster robust**:
...
A sequence of random vectors $Z_n \in \mathbb{R}^K$ **converges in probability** to a random vector $Z$ (in that case, we say that $Z_n \rightarrow_p Z$ or $\text{plim}_{n \in \mathbb{N}} Z_n = Z$), if for all $\delta > 0$, there is an $\epsilon >0$ and a $N$ such that for all $n > N$: $$\text{Pr}\left( ||Z_n - Z|| \leq \delta \right) > 1 - \epsilon$$
...
since the second summation converges to $\mathbf{E} \left[ XX^{\prime}  \right]^{-1} \mathbb{E} \left[ e | X \right] = 0$ $\blacksquare$
...
$$\sqrt{n} \left( \frac{1}{n} \sum_{i=1}^{n} X_ie_i - \mathbb{E} \left[ e | X \right] \right) \sim_{ass} N\left(0, \mathbb{E}\left[ XX^{\prime}e^2\right] \right)$$

---
class: middle
## Asymptotic normality

Finally, we apply the **Delta Method**:
...
Note comparing the formulas that $n \widehat{V}_{\widehat{\beta}}^{\text{HC0}} \rightarrow \text{Avar}(\widehat{\beta})$: because of the **WLLN**, $\mathbb{E}\left[ XX^{\prime}e^2\right] \rightarrow_p 0$, so to have a non-degenerate variance $\text{Avar}(\widehat{\beta})$, we multiply $\widehat{\beta} - \beta$ by $\sqrt{n}$ (and thus its variance by $n$ because of bilinearity)
...
Note that all tests assume $H_0$ is true (condition on it) &mdash; frequently, the (asymptotic) distribution of the test is not even known under $H_1$
...
$$\pi(\theta) = \Pr (\text{reject } H_0 \ |\ H_1) = \Pr (T > c\ |\ H_1)$$
...
We set the **statistical significance** $\alpha$ to ensure the size is at most $\alpha$, so when we reject $H_0$ we can say the data is inconsistent with the null &mdash; but when we do not reject $H_0$, we frequently cannot "accept" the null, since *we do not know the power of the test*

There is a difference between *statistically* significant effects and **economically significant effects**: nowadays, with gigantic data-sets, we often can reject the null even with trivially small (but positive) effects
...
Frequently, researchs present tables with tens or (I have seen it before) hundreds of different coefficients and their statistical significance (therefore implying a hypothesis test)
...
This is handled (among others) by the **Bonferroni correction**: if we are testing $k$ statistics, we can bound the size of the test of *any* rejection by $\alpha$ if we reject $H_0$ only if the smallest p-value is smaller than $\alpha/k$ 
