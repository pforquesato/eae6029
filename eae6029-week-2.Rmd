...
date: "2024/1 - Topic 2: Causality and instruments"
output:
...
knitr::include_graphics("figs/collider.jpg")
```

Another example of collider bias, this time from real research: the effect of a booster shot on a omicron infection is confounded by the problem that people are (presumably) more likely to take a booster if infected, and this affects the reinfection probability &mdash; so we just control for previous infection and we are fine, right? *Wrong*, since that introduces a **collider bias**  [@_MiguelHernan](https://twitter.com/_MiguelHernan/status/1670795479326531585?s=20)
...
Another source of causal bias not mentioned above is known as **reverse causality bias**: when we presume that a statistic association (correlation) between $A$ and $B$ means $A \rightarrow B$, when in fact it is the case that $B \rightarrow A$

Since causality is mediated by time, reverse causality is usually present only in more coarse types of time data, like yearly aggregate data (GDP, inflation, etc.) in macroeconometrics

But sometimes we can have a two-way causal relations even on the same period through beliefs or expectations
...
Therefore, the *reduced regression* linear projection of $Y_1$ on $Z$, $Y_1 = Z^{\prime}\lambda + v$ identifies the parameters $\lambda_1 = \beta_1 + \beta_2\gamma_1$ and $\lambda_2 = \beta_2\gamma_2$ (note that if $k_2=l_2=1$, then $\beta_2 = \lambda_2/\gamma_2$)
...
Now call $\Gamma$ the coefficient matrix of the total projection of $X = [Z_1, Y_2]$ on $Z = [Z_1, Z_2]$: since projecting $Z_1$ on $Z_1$ is themselves, we have that: 

$$\Gamma = \left[ \begin{matrix} I_{k_1} \ \ \ \ \gamma_1 \\ 0_{l_2} \ \ \ \  \gamma_2 \end{matrix} \right]$$

Remember that the **inclusion restriction** was that $\mathbb{E}[ZX^{\prime}]$ is rank $k$: given linear independent $Z$, this will happen if and only if $\Gamma$ has full rank $k$, or namely $\gamma_2$ of size $l_2$ has rank $k_2$
...
Note that when the model is overidentified, the covariance between residuals and instruments $\mathbf{Z}\widehat{\mathbf{e}} \neq 0$ &mdash; in fact, since it is a linear system with $k$ equations and $l > k$ unknowns, there is *no* solution to the equality

This will lead to the Generalized Method of Moments estimator, that finds the solution for $\mathbf{Z}\widehat{\mathbf{e}}$ closest to zero &mdash; but that is a topic for EAE-6030
...
Also, under the null of exogenous $Y_2$, both OLS and 2SLS are consistent, so they must converge to the same value! &mdash; in the end, both approaches are equivalent: the **Durbin-Wu-Hausman endogeneity tests** 
...
We also require a **monotonicity condition**: that there are no **defiers**, people that will go to college if they live far, but not if live close &mdash; for all $U$, $g(1, U) > g(0, U)$
...
Indeed, we can always write $$X = (1 - Z)X(0) + ZX(1) = X(0) + Z\left(X(1) - X(0)\right)$$

Analogously, $Y = Y(0) + XC = Y(0) + X(0)C + ZC\left(X(1) - X(0)\right)$, which gives us, using that $Z \perp \!\!\! \perp U \Rightarrow Z  \perp \!\!\! \perp Y(1), Y(0), X(1), X(0)$  $$\mathbb{E}[Y|Z=1] - \mathbb{E}[Y|Z=0] = \mathbb{E}[C\left(X(1) - X(0)\right)]$$

$$= \mathbb{E}[C\left(X(1) - X(0)\right)| \ X_i(1) - X_i (0) = 1] \Pr(X_i(1) - X_i (0) = 1) $$
$$+\mathbb{E}[C\left(X(1) - X(0)\right)| \ X_i(1) - X_i (0) = 0] \Pr(X_i(1) - X_i (0) = 0)$$
$$+\mathbb{E}[C\left(X(1) - X(0)\right)| \ X_i(1) - X_i (0) = -1] \Pr(X_i(1) - X_i (0) = -1) \ \blacksquare$$
