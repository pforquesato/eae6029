...
---
class: middle
## Example: Harding et al (2012)

To investigate geographic incidence, they add distance to border $D_{ijt}$ and interact it with tax difference among states $(\tau^h - \tau^b )_{jt}$, comparing people that live close to the border with those far, and those that live in borders with no tax difference with those with tax difference:
...
Now the effect of taxes on prices varies by location &mdash; to interpret the results, we need to consider that the **partial effects** are:
